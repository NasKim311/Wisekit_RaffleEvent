...
public class RaffleService {
    ...
    // 순위별 당첨자 수 (해당 순위가 추첨될 때마다 1씩 증가)
    private int firstWinnerByLotCnt = 0;
    private int secontWinnerByLotCnt = 0;
    private int thirdWinnerByLotCnt = 0;

    // 응모자 인원 (응모할 때마다 1씩 증가)
...
//    @Transactional
    public int byLot() {
        ...
        //   표 추첨 메소드 실행 시 응모자 인원 추가
        applicantCount++;

        // 응모자가 일평균 당첨자보다 초과인 경우
        if (applicantCount > dayAverageAllWinnerCount * ongoingPeriod) {
            // 1등이 추첨되었지만 일단위 1등이 모두 당첨된 경우 4등만 당첨
            if (rankByLot(byLotNum) == 1 && firstWinnerByLotCnt > dayAverageFirstWinnerCount * ongoingPeriod) {
                byLotNum = fourthWinnerByLot;

            // 2등이 추첨되었지만 일단위 2등이 모두 당첨된 경우 4등만 당첨
            } else if (rankByLot(byLotNum) == 2 && secontWinnerByLotCnt > dayAverageSecondWinnerCount * ongoingPeriod) {
                byLotNum = fourthWinnerByLot;

            // 3등이 추첨되었지만 일단위 3등이 모두 당첨된 경우 4등만 당첨
            } else if (rankByLot(byLotNum) == 3 && thirdWinnerByLotCnt > dayAverageThirdWinnerCount * ongoingPeriod) {
                byLotNum = fourthWinnerByLot;
            }
